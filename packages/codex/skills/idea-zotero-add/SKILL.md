...
description: Use this skill when the user invokes $idea-zotero-add, /idea-zotero-add, asks to run idea-zotero-add, or asks to add a paper to the Zotero collection for an idea. This is the Codex-native copy of the AI Research Tools workflow; do not read Claude command files at runtime.
---
# idea-zotero-add
...
- $idea-zotero-add
- /idea-zotero-add
- Natural language requests to add a paper to the Zotero collection for an idea
## Codex Execution Rules
- Do **not** read {{HOME}}\.claude\commands\idea-zotero-add.md at runtime. This skill is the copied command source for Codex.
...
- api_key
- user_id  
- idea_collections map (slug 鈫?collection key)
...
- In the Literature Exploration section, find the paper and add a 鉁?Zotero marker next to it
- If the paper wasn't in the list yet, add it to a "## Added to Zotero" subsection
- Append to `ideas/log.md`: `[ZOTERO-ADD YYYY-MM-DD] slug: <slug> 鈫?added: <paper title>`
