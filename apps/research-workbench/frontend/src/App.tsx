...
import {
  ...
  CircleCheck, Clock3, FileText, GitBranch, HeartPulse, Home, Layers3, Lightbulb,
  ListChecks, LoaderCircle, Menu, MessageSquareText, Moon, Play, RefreshCw, Search,
...
import type {
  ...
  Paper, ReadingSession, RunReceipt, Slate, WeeklyPlan,
} from "./types";
...
function PaperCard({ paper, rank, onOpen, onAction, busy }: {
  ...
  return <article className="paper-card">
    ...
    <div className="paper-card-body">
      ...
      <p className="reason">{paper.public_reason || paper.relevance_reason || "等待本地 Codex 生成推荐理由。"}</p>
...
export default function App() {
  ...
  useEffect(() => {
    if (!week) return;
    if (nav === "papers") get<Paper[]>(`/api/weeks/${week}/papers`).then(setPapers).catch(showError);
  ...
  async function paperAction(paper: Paper, action: string, clusterId = "") {
    ...
    try {
      ...
      if (nav === "papers") setPapers(await get<Paper[]>(`/api/weeks/${week}/papers`));
    } catch (reason) { showError(reason); }
    finally { setBusy(""); }
  }
  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      <div className={`page page-${nav}`}>
        {nav === "week" && dashboard && <WeekView dashboard={dashboard} busy={busy} onOpen={openPaper} onAction={paperAction} onRank={rankWeek} onConfirmPlan={confirmPlan} onUpdatePlan={updatePlan} onDraftPlan={draftPlan} onClusters={proposeClusters} onClusterStatus={setClusterStatus} onApproval={answerApproval} />}
        ...
        {nav === "reading" && <ReadingView paper={selectedPaper} session={session} busy={busy} onAction={paperAction} setSession={setSession} showError={showError} />}
...
function WeekView({ dashboard, busy, onOpen, onAction, onRank, onConfirmPlan, onUpdatePlan, onDraftPlan, onClusters, onClusterStatus, onApproval }: {
  dashboard: Dashboard; busy: string; onOpen: (paper: Paper) => void; onAction: (paper: Paper, action: string, cluster?: string) => void;
  ...
  return <div className="dashboard-grid">
    <section className="top5-panel panel">
      <SectionTitle eyebrow="WEEKLY SLATE" title="本周推荐的五篇论文" action={<button className="secondary" disabled={!!busy} onClick={onRank}>{busy === "rank" ? <LoaderCircle className="spin" size={15} /> : <Sparkles size={15} />}Codex 重新排名</button>} />
      <div className="slate-note"><span className="status-dot" />{dashboard.slate.generated_by === "codex-app-server" ? "已由本地 Codex 排名" : "当前为确定性预排；登录后可用 Codex 个性化排名"}<span>· 完成后自动补位</span></div>
      <div className="paper-stack">
        {dashboard.top5.length ? dashboard.top5.map((paper, index) => <PaperCard key={paper.paper_id} paper={paper} rank={index + 1} onOpen={() => onOpen(paper)} onAction={(action) => onAction(paper, action)} busy={busy.startsWith(paper.paper_id)} />) : <Empty title="本周没有候选论文" detail="运行 Paper Tracker discovery-only 后，完整候选池会出现在这里。" />}
      </div>
    </section>
...
function PapersView({ papers, query, setQuery, busy, onOpen, onAction }: {
  ...
  useEffect(() => setVisibleCount(40), [query, papers.length]);
  ...
  return <section className="panel library-panel">
    ...
    <div className="filter-row"><button className="filter active">全部 <span>{papers.length}</span></button><button className="filter">阅读中</button><button className="filter">已归档</button><button className="filter">Backlog</button></div>
    <div className="paper-table">
      ...
      {!papers.length && <Empty title="没有匹配的论文" detail="换一个关键词，或检查本周候选池是否已同步。" />}
      {visibleCount < papers.length && <button className="secondary load-more" onClick={() => setVisibleCount((count) => count + 40)}>再显示 40 篇 <span>{visibleCount} / {papers.length}</span></button>}
    </div>
  </section>;
}

function ReadingView({ paper, session, busy, onAction, setSession, showError }: {
  paper: Paper | null; session: ReadingSession | null; busy: string; onAction: (paper: Paper, action: string) => void;
  ...
  if (!paper) return <Empty icon={BookOpen} title="先选择一篇论文" detail="从“本周”或“论文”打开论文后，这里会保留 PDF、Codex 对话与阅读阶段。" />;
