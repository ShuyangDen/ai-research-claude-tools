  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (boot.frontend_version && boot.frontend_version !== WORKBENCH_UI_VERSION) {
          const target = new URL(window.location.href);
          target.searchParams.set("ui", boot.frontend_version);
    ...
    return <div className="project-editor-backdrop" role="presentation" onMouseDown={() => setDraft(null)}>
      <form className="project-editor panel" onSubmit={submit} onMouseDown={(event) => event.stopPropagation()}>
        ...
        <div className="project-form-grid">
          ...
          <label className="full"><span>{pick("本机项目路径", "Local project path")}</span><input required value={draft.project_path} onChange={(event) => setDraft({ ...draft, project_path: event.target.value })} placeholder="D:\\Research\\example-project" /></label>
...
function RunsView({ runs, syncOverview, onSync, onResume, busy }: {
  ...
  return <div className="runs-layout">
    <section className="panel sync-panel">
      ...
      <div className="sync-boundary"><ShieldCheck size={18} /><div><strong>{pick("只同步已提交的 Git 内容", "Only committed Git content is transferred")}</strong><span>{pick("普通仓库有未提交改动时仍可同步：Git 会保留这些改动，只安全拉取和推送已提交历史。Workbench Private State 会在手动同步时自动提交。", "Repositories remain syncable with local edits: Git preserves them and only transfers committed history when safe. Workbench Private State is auto-committed during manual sync.")}</span></div></div>
      <div className="sync-grid">
        {syncOverview?.repositories.map((repository) => {
          ...
          return <article className={`sync-repository state-${repository.state}`} key={repository.repository_id}>
            ...
            <button className="secondary wide" disabled={syncBusy || blocked} title={blocked ? pick("请先配置可用的 upstream", "Configure an available upstream first") : pick("保留本地未提交改动，并安全同步已提交历史", "Preserve local edits and safely sync committed history")} onClick={() => onSync([repository.repository_id])}>{busy === `sync:${repository.repository_id}` ? <LoaderCircle className="spin" size={14} /> : <RefreshCw size={14} />}{pick("同步这个仓库", "Sync this repository")}</button>
          </article>;
        })}
